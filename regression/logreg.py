...
#clinical_data = pd.read_csv('./data/processed/baseline_clinical_data_imputed_knn.csv', index_col=0)
...
y = patient_endp[['D_PT_lstalive', 'D_PT_deathdy']].max(axis=1).loc[all_ids]
y = y.loc[pd.notnull(y)]
y = y>365.
...
logreg = LogisticRegression(penalty='l2', random_state=1, max_iter=10000, n_jobs=3)
